...
import com.appotronics.appo_lib.AppoCommand
...
class MainActivity : AppCompatActivity() {
    ...
    fun appoResponse(view: View?) {
        AppoSDK.appoResponse(object : AppoResponseCallback {
            override fun response(json: String) {
                ...
                if (parseObject != null) {
                    startApp(parseObject.type, parseObject.url)
                }
            }
        })
    }
    ...
    private fun startApp(type: String, mediaUrl: String) {
        val intent = Intent(Intent.ACTION_MAIN)
        intent.addCategory(Intent.CATEGORY_LAUNCHER)
        intent.flags = Intent.FLAG_ACTIVITY_NEW_TASK
        intent.putExtra("type", type)
        intent.putExtra("content", mediaUrl)
